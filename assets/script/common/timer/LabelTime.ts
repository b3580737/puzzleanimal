...
export default class LabelTime extends Label {
    ...
    setTimeStamp(timeStamp: number) {
        this.countDown = TimeUtil.secsBetween(Global.timer.getServerTime(), timeStamp);
        this.timing_end();
        this.timing_start();
        this.format();
    }
    ...
    onLoad() {
        if (!EDITOR) {
            Global.message.on(EventMessage.GAME_SHOW, this.onGameShow, this);
            Global.message.on(EventMessage.GAME_HIDE, this.onGameHide, this);
        }
    }
    ...
    onDestroy() {
        if (!EDITOR) {
            Global.message.off(EventMessage.GAME_SHOW, this.onGameShow, this);
            Global.message.off(EventMessage.GAME_HIDE, this.onGameHide, this);
        }
    }
    ...
    private onGameShow() {
        ...
        const interval = Math.floor((Global.timer.getTime() - (this.backStartTime || Global.timer.getTime())) / 1000);
    ...
    private onGameHide() {
        this.backStartTime = Global.timer.getTime();
    }
